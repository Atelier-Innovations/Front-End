...
app.get('/products/:id/related', (req: Request, res: Response) => {
  const id = req.params.id
  console.log('this is the related products route')
  console.log('id: ', id)
  axios(`https://app-hrsei-api.herokuapp.com/api/fec2/hr-rfe/products/${id}/related`, {
    headers: {
      Authorization: process.env.API_KEY
    }
  })
  .then(results => {
    console.log('related products: ', results.data)
    res.send(results.data)
  })
})
